package com.example.demo.model;

import jakarta.persistence.*;
import lombok.Data;
import org.hibernate.annotations.GenericGenerator;

import java.util.List;
import java.util.UUID;

@Entity
@Table(name = "Chat_Lieu")
@Data
public class ChatLieu {

    @Id
    @GeneratedValue(generator = "uuid2")
    @GenericGenerator(name = "uuid2", strategy = "org.hibernate.id.UUIDGenerator")
    @Column(name = "chat_lieu_id", columnDefinition = "uniqueidentifier")
    private UUID chatLieuId;

    @Column(name = "ten_chat_lieu", nullable = false, length = 255, columnDefinition = "NVARCHAR(255)") // Thêm columnDefinition = "NVARCHAR(255)"
    private String tenChatLieu;

    @Column(name = "trang_thai", length = 50, columnDefinition = "NVARCHAR(50)") // Thêm columnDefinition = "NVARCHAR(50)"
    private String trangThai;

    @OneToMany(mappedBy = "chatLieu", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<SanPham> sanPhams;
}